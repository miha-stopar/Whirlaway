...
- Progress: both device runtimes now cache their device/pipeline setup across AIR evaluations, so zerocheck no longer recompiles shaders or recreates queues on every invocation.
- Progress: the GPU path now splits the AIR payload into a cached static program section plus per-call dynamic data, so canonical opcode encoding and immutable instruction/constant/output uploads are no longer repeated on every hypercube evaluation.
- Progress: the `Metal` AIR kernel now uses native 64-bit multiplication for modular products instead of the earlier bit-serial fallback.
...
- Next bottleneck: after caching the runtime objects, static program encoding, and immutable program buffers, the likely remaining costs are building `flat_inputs` on the host and uploading the per-call dynamic buffers (`header`, `batching_scalars`, `flat_inputs`, and `results`).
